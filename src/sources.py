...
class Article:
    ...
    score: float = 0.0
    # عوامل الجذب التحريرية الثلاثة (Issue #876) — يقدّرها نداء الفرز
    # screen.screen القائم نفسه بلا نداء إضافي، كل واحد 0-3 منفصل عمدًا
    # عن الآخرين. القيمة الافتراضية 0 تعني الحياد التام: خبر لم يُفرز بعد
    # (أو فشل الفرز) يمر بترتيبه الحالي بلا أي أثر لهذه الحقول.
    impact: int = 0                  # أثر معيشي مباشر على القارئ العربي
