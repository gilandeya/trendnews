...
# سقف مساهمة _relevance في الدرجة المركّبة (تشخيص Issue #373، تعليق
# الموافقة الخامس عشر، البند 1): _relevance بلا سقف بينما فارق الوزن بين
# TRUSTED_PUBLISHER_WEIGHT وDEFAULT_PUBLISHER_WEIGHT ثابت (2.4) — رصد
# top_candidates (البند 2 من التشخيص السابق) حسم الفرضية برقم فعلي من
# تشغيل حقيقي: "تطبيق نبض" (وزن 0.6، صلة 4) هزم "برس بي" (وزن 3.0، صلة 1)
# بدرجة 4.6 مقابل 4.0 — مطابقة لفظية لأربع كلمات من الاستعلام كافية لهزيمة
# وكالة موثوقة، نفس شكل فشل 365Scores الأصلي لكن في طبقة الدرجة المركّبة لا
# فلتر relevant().
#
# القيمة 3.0 مشتقة من نافذة صالحة محسوبة من الشاهدين معًا لا اختيارًا
# اعتباطيًا: يجب أن يبقى مرشّح افتراضي الوزن (0.6) بصلة معتدلة (4) *مهزومًا*
# أمام موثوق (3.0) بصلة ضعيفة (1) — يتطلب سقفًا ≤ 3.4 (0.6+سقف ≤ 3.0+1) —
# وأن يبقى مرشّح افتراضي الوزن بصلة شديدة الارتفاع (قريبة من كل كلمات
# الاستعلام) قادرًا على الفوز على موثوق بلا أي صلة إطلاقًا (شاهد #132: لا
# يجوز أن يُقصى مرشّح شديد الصلة كليًا — العلة الأصلية التي بُني الجمع
# لعلاجها) — يتطلب سقفًا > 2.4 (0.6+سقف > 3.0+0). النافذة الصالحة إذن
# (2.4, 3.4]؛ 3.0 تقع في وسطها بهامش أمان في الاتجاهين، وتساوي
# TRUSTED_PUBLISHER_WEIGHT نفسها — قراءة مقصودة: مساهمة الصلة وحدها لا يمكن
# أن تتجاوز ما يمنحه كون المصدر وكالة موثوقة بذاتها.
RELEVANCE_CAP = 3.0
...
def _candidate_score(weight: float, relevance: int) -> float:
    ...
    الصلة تُقصّ عند RELEVANCE_CAP (انظر توثيقها) قبل الجمع — بلا هذا القص،
    مصدر افتراضي الوزن بمطابقة لفظية كافية لأربع كلمات من الاستعلام كان
...
def _candidate_sort_key(weight: float, relevance: int) -> tuple[float, float]:
    """مفتاح فرز مرشّحي القراءة: الدرجة المركّبة (_candidate_score) أولًا،
    ثم الوزن فاصلًا عند تعادلها تمامًا — لا ترتيب الوصول الموروث من
    search()/rank() (تشخيص Issue #373، تعليق الموافقة الخامس عشر، البند 1:
    شاهد حقيقي رصد خمسة مرشحين افتراضيي الوزن بدرجة مركّبة متطابقة تمامًا
    قبل هذا الإصلاح، حُسم وقتها باختيار عشوائي فعليًا لا حكمًا). دالّة
    مستقلة (لا lambda مضمَّنة في candidates.sort) لتبقى قابلة للاختبار
    مباشرة بمعزل عن machinery gather_evidence الكاملة."""
    return (-_candidate_score(weight, relevance), -weight)
...
def gather_evidence(articles: list[Article], cfg, claim_text: str = "",
    ...
    # عند تعادل الدرجة المركّبة تمامًا (شائع بعد قصّ الصلة أعلاه — عدة
    # مرشحين افتراضيي الوزن يبلغون السقف معًا)، الوزن هو الفاصل الأول (انظر
    # _candidate_sort_key)
    candidates.sort(key=lambda c: _candidate_sort_key(c[0], c[1]))
