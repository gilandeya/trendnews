...
الأنبوب (كل خطوة موثَّقة في الدالة المسؤولة عنها):
  ...
  4) _sufficiency() — تتحقق فقط من وجود واقعة واحدة مسندة على الأقل ضمن
     الوقائع **المُرشَّحة بالسند فقط** (القاعدة 7 القديمة — بوابة عددية/نوعية
     ثلاثية — أُلغيت؛ انظر CLAUDE.md وسدّ ثغرة الدائرة في التوثيق أدناه)
...
def _sufficiency(grounded: list[dict], cfg) -> tuple[bool, str]:
    """بوابة الكفاية — بعد إلغاء القاعدة 7 (Issue #814، جزء 1 من 3): كانت
    هذه الدالة تضم ثلاث بوابات امتناع (عدد الوقائع المسندة دون
    `min_grounded_facts`، كل الوقائع المسندة مرجعية، كل الوقائع المسندة
    تقارير منقولة) تمتنع عن المقال كليًا حين لا تُجتاز — فيصمت المسار عن
    القارئ بدل أن يقول له ما تأكّد وما لم يتأكّد. أُلغيت الثلاثة معًا: موجز
    محرِّر بشري قد يحمل أرقامًا صحيحة غير مفهرسة في الأخبار (مثال حيّ:
    خسارة ربع أول وعدد موظفين وحجم دين شركة بعينها) فتسقط كل وقائعه
    فرادى دون أن يكون ذلك خطأً في الموجز — والصمت ليس الجواب الصحيح؛ منشور
    التحقيق (`draft_investigation`) هو من يحمل الآن ما لم يتأكّد، بلا حاجة
    لامتناع المقال عنه أيضًا. `min_grounded_facts` بقي في config.yaml بلا
    استعمال عمدًا (رصيد توثيقي لو استُؤنفت العتبة العددية يومًا).

    الشرط الوحيد الباقي عددي محض ولا علاقة له بجودة الوقائع: إن لم تحصل ولا
    واقعة واحدة على مصدر مستقل أو أكثر، فلا مادة للمقال إطلاقًا — لا وقائع
    ...
    **ترتيب حاسم يبقى كما هو** (سدّ ثغرة الدائرة، آخر تعليق على Issue #348):
    ...
    يستدعي هذه الدالة بعد حلقة السند مباشرة، وقبل _choose_question."""
    if not grounded:
        return False, "لا واقعة واحدة حصلت على مصدر مستقل أو أكثر — لا مادة لمقال"
...
def _write_article(body: str, issue_number: int, cfg) -> dict:
    ...
    # sources مكتملة حتى حين يمتنع المقال في أي بوابة أدناه — sources_seen
    # مكتملة هنا فعلًا (كل حلقات السند أعلاه انتهت)، لا تتغيّر لاحقًا
...
def draft_investigation(outcome: dict, cfg) -> dict | None:
    ...
    تُستدعى في كل تشغيلة (Issue #814، جزء 1 من 3) — بصرف النظر عن
    outcome['produced']: نجاح المقال أو فشله (بأي بوابة: كفاية، صياغة،
    نسبة، أصالة) لا يمنع إنتاج التحقيق، لأن dropped/diffs/sources تُبنى
    أثناء حلقة السند نفسها لا بعد نجاح المقال — راجع _write_article. لا
    تحقيق فقط إن لم يكن هناك ما يُحقَّق فعلًا (dropped وdiffs كلاهما فارغ)
    — لا معنى لمنشور يقول «كل شيء صحيح»، هذا ليس تحقيقًا (سلوك #765 قائم
    كما هو، لم يتغيّر هنا).
...
def build_report(outcome: dict, investigation: dict | None = None) -> str:
    ...
    if outcome["produced"]:
        ...
        if investigation:
            # لا مقال هنا (Issue #814، جزء 1) لكن التحقيق قد يُنتَج رغم ذلك —
            # نظير سطر «صيغ أيضًا» أعلاه، بلا الاعتماد على نجاح المقال
            lines.append(f"🔎 صيغ مع ذلك منشور تحقيق من الادّعاءات غير المسندة/المختلَفة "
                         f"عن الموجز (المعرّف `{investigation['id']}`).")
