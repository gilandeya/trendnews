...
د. حقل dispute للتناقض الحقيقي **فقط**. قبل أن تكتب فيه حرفًا، وحّد
   الوحدات والصياغات واسأل: هل يستحيل أن يكون القولان صحيحين معًا؟
   ...
   إن انتهيت إلى أن الفرق شكلي، اترك dispute **فارغًا** ولا تشرح لماذا.
   ⛔ **وإن كنت قد ذكرت طرفَي الخلاف في post_body، اترك dispute فارغًا.**
   تكراره بصياغة أخرى حشو يكشف أن النص مركّب من حقول لا مكتوب ككل.
   اقرأ متنك قبل ملء الحقل: هل يعرف القارئ الخلاف منه؟ إن نعم، فاسكت.
   dispute لما لم يتّسع له المتن فقط.
   ⛔ ممنوع منعًا باتًا أن تكتب في dispute ثم تنفي التناقض داخله بعبارات
   مثل "لا تناقض فعليًا" أو "رقمان متقاربان" — إن كتبتَ ذلك فقد أخطأت
   بالكتابة أصلًا. الحقل الفارغ هو الجواب الصحيح.
   ولا ترجّح طرفًا حين يكون التناقض حقيقيًا — انقل القولين بلا حكم.
...
def write_arabic(article: Article, cfg, retries: int = 3,
    ...
    # عبارات تدل على أن النموذج كتب "خلافًا" ثم نفاه — الحقل حينها لغو
    SELF_NEGATING = (
        "لا تناقض", "ليس تناقض", "لا يوجد تناقض", "لا اختلاف", "متقاربان",
        "متطابقان", "نفس ", "المعنى نفسه", "لا يتعارض", "غير متعارض",
        "بوحدتين مختلفتين", "لا خلاف",
    )
    ...
    def clean_dispute() -> str:
        """يرفض الخلاف الذي ينفي نفسه — الحقل الفارغ أصدق منه."""
        value = clean("dispute")
        if value and any(marker in value for marker in SELF_NEGATING):
            log.info("أُلغي حقل الخلاف: النموذج نفى التناقض داخله")
            return ""
        return value
    ...
    return {
        ...
        "dispute": clean_dispute() if len(source_docs or []) >= 2 else "",
...
def build_caption(written: dict, article: Article, cfg) -> str:
    ...
    if written.get("dispute"):
        # بلا عنوان: الإعلان عن أهمية ما ستقوله بصمة آلية. الخلاف يُذكر
        # كجملة عادية ضمن النص، والقارئ يفهمه من صياغته.
        lines += ["", written["dispute"]]
