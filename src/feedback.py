...
REASONS: dict[str, str] = {
    ...
    # مرحلة الاختيار قبل الصياغة (Issue #280): مرشّح ترك بلا تعليم صراحة —
    # سبب عام لا يعني عيبًا فيه، فقط أن دفعته الأخرى كانت أولى بالاختيار.
    "لم يُختر": "لم يكن ضمن المختار في دفعة الاختيار",
    # عدم الاعتماد = رفض ضمني (Issue #841): لا خيارات سبب استبعاد في واجهة
    # المراجعة بعد اليوم — المدخلة تُسجَّل بلا سبب حقيقي بعد، والتقرير
    # الأسبوعي (مهمة تالية) سيسأل عن السبب ويحوّله إلى وسم حقيقي من هذه
    # القائمة. تُستبعد عمدًا من screening_guidance أدناه لأنها لا تصف شيئًا.
    "لم يُعتمد": "لم يُعتمد داخل Issue المراجعة — السبب الحقيقي غير معروف بعد",
}

# وسما «لا سبب حقيقي بعد» — يقولان «لم يُنشر» لا «لماذا» (Issue #843).
# مستوى الوحدة لا محليًا داخل screening_guidance وحدها: src/insights.py
# (قسم «لماذا لم تنشر هذه؟») يحتاج نفس المجموعة كي يعرض مدخلاتهما فقط
# وليعرف أي وسم مُجاب يُستثنى من إعادة الاستثناء بعد الإسناد.
NON_REASON_TAGS: set[str] = {"لم يُعتمد", "لم يُختر"}
...
def screening_guidance(entries: list[dict], limit: int = 12,
    ...
    مدخلات NON_REASON_TAGS («لم يُعتمد» و«لم يُختر»، Issue #841، وتصحيح Issue #843:
    الاستبعاد الأول شمل «لم يُعتمد» وحدها ونسي «لم يُختر» رغم أنها تقول
    الشيء نفسه بالضبط — «لم يُنشر» لا «لماذا». عدم الاعتماد داخل Issue
    المراجعة وعدم الاختيار ضمن دفعة الاختيار كلاهما يُسجَّل رفضًا تلقائيًا
    بلا سبب حقيقي) تُستبعد أيضًا: تمريرها هنا يملأ توجيه الفرز بعنوان خبر
    بلا أي سبب يصفه، فيفسد التوجيه بدل أن يحسّنه. القياس الذي كشف الخلل:
    ٦٦٤ من ٧٠٠ مدخلة في state/rejections.json موسومة «لم يُختر»، فخانات
    التوجيه الاثنتا عشرة كانت تمتلئ بها بلا أي سبب حقيقي.
    """
    ...
    recent = [
        ...
        and e.get("tag") not in NON_REASON_TAGS
    ][-limit:]
    if not recent:
        return ""
